...
下一任务：P7 前端完整可用（BUILD_ORDER §P7：登录 / 工作台 / 任务 / 调度 / 黑板可视化 / 复盘等页面）。注：P6.4 / P6.5 数据集下载 + Colab GPU 训练 由用户独立完成；落地时只需替换 `app/api/v1/perception.py::_mock_infer` 为 ultralytics YOLO；P6.9 Mock 视觉数据流已就绪（`mock_perception_enabled=True` + `mock_perception_detection_rate=0.05~0.2` 即可演示完整链路：has_yolo UAV → Mock detection → 黑板 fuse → WS perception.detection → 高置信 survivor 自动派任务 → P5.7 自动拍卖 → P6.8 vision_boost ×1.5）。  
