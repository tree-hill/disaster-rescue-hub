...
下一任务：P7.2 前端基础设施（BUILD_ORDER §P7.2：axios 拦截器 + Zustand auth/ws store + 受保护路由）→ P7.3 6 大页面（Login/Cockpit/RobotManagement/TaskManagement/Blackboard/AlertCenter/Admin）。P7.1 后端已就绪：前端 Cockpit 顶条调 `GET /situation/kpi` + 订阅 WS `kpi.snapshot`（commander 房间）；告警中心调 `GET /alerts?status=unack` + 订阅 `alert.raised/acknowledged/ignored`；指挥员 ack/ignore 走 `POST /alerts/{id}/acknowledge` / `/ignore`。注：P6.4 / P6.5 数据集下载 + Colab GPU 训练 由用户独立完成；落地时只需替换 `app/api/v1/perception.py::_mock_infer` 为 ultralytics YOLO；P6.9 Mock 视觉数据流已就绪。  
