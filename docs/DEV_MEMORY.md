...
下一任务：P6.9 Mock 视觉数据流（BUILD_ORDER §P6.9：UAV Agent 1Hz 调 PerceptionService.process_image，AIDER 测试集随机抽图模拟摄像头帧）。注：P6.4 数据集下载/标注 + P6.5 Colab GPU 训练由用户独立完成；P6.6/P6.7/P6.8 backend 链路已就绪，best.pt 落地时只需替换 `app/api/v1/perception.py::_mock_infer` 为真实 ultralytics YOLO 调用。  
