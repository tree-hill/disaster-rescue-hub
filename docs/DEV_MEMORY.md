...
下一任务：P7.3 6 大页面实现（BUILD_ORDER §P7.3：Login / Cockpit / RobotManagement / TaskManagement / Blackboard / AlertCenter / Admin）。P7.2 已搭好：路由 + auth/ws store + 受保护路由就绪，P7.3 在占位页基础上接入数据；Login 页面登录成功后调 `useAuthStore.setSession({user, accessToken, refreshToken})` 然后 navigate('/cockpit')；Cockpit 顶条 `GET /situation/kpi` + WS 订阅 commander 房间收 `kpi.snapshot`；告警中心 `GET /alerts?status=unack` + 订阅 `alert.raised/acknowledged/ignored`；ack/ignore → `POST /alerts/{id}/acknowledge` / `/ignore`。WS 客户端 emit 'subscribe' {rooms:['commander']}；后端只允许 commander/admin 角色。注：P6.4 / P6.5 数据集下载 + Colab GPU 训练 由用户独立完成；落地时只需替换 `app/api/v1/perception.py::_mock_infer` 为 ultralytics YOLO；P6.9 Mock 视觉数据流已就绪。  
