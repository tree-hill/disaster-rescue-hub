...
当前阶段：P5 调度算法（P5.1~P5.8 全部完成，下阶段进入 P6 视觉感知）  
当前任务：P6.1 黑板系统（BUILD_ORDER §P6.1）  
任务来源：docs/BUILD_ORDER.md  
备注：P5.8 跑通所有测试用例完成（pytest 8.0.2 + pytest-asyncio 0.23.8 装入 venv；backend/tests/algorithms/ 落地 TC-1~TC-10 共 10 个纯函数测试；backend/tests/e2e/ 落地 TC-E2E-1 任务全生命周期 + TC-E2E-2 HITL 改派完整链路；E2E function-scoped autouse fixture + engine.dispose 解决 pytest-asyncio 0.23 跨 loop 连接池问题；12/12 全绿 2.46s）。  
...
- [ ] P6.1 黑板系统（BUILD_ORDER §P6.1）：DATA_CONTRACTS §1.x blackboard 表 + Blackboard 服务（write/query_by_proximity/TTL 清理）
