...
当前阶段：P5 调度算法（P5.1~P5.7 已完成，拍卖闭环 + REST + HITL 改派 + 自动触发）  
当前任务：P5.8 跑通所有测试用例（BUILD_ORDER §P5.8）  
任务来源：docs/BUILD_ORDER.md  
备注：P5.7 任务自动触发拍卖完成（dispatch_trigger.py：_on_task_created handler 订阅 task.created 走 child_count 分支，父跳过+子逐个 + PendingAuctionScanner 30s 间隔异步循环 + asyncio.Event 优雅停 + 单例；TaskRepository.find_pending_leaves NOT IN 父集合；config 加 dispatch_auto_trigger_enabled / dispatch_pending_scan_interval_sec；main.py lifespan 串起 trigger + scanner；17/17 自检全绿）。  
